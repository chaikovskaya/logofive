...
/*    $( document ).ready(function(){
                ...
                selectHead.on("click", function () {
                    ...
                    if (!$(this).hasClass("on")) {
                        ...
                        selectItem.on("click", function () {
                            ...
                            //console.log( 'клик по - ' + $(this).attr('id') );
                            //console.log( 'нужный селект id - ' + $(this).parent().parent().find('select').attr('id') );
        ...
        function PopUpShow(popup, video_id ) {
            ...
            if( video_id ){ $('#' + popup + ' ._pop_window .video_container').append('<iframe src="https://www.youtube.com/embed/' + video_id + '" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>'); }
...
*/
            ...
            // анимация счетчиков если они есть на странице и уже на экране
            if ($('.counters .wrapper ul').length > 0) { if(elem_in_visible_area('.counters .wrapper ul')){ if(animatioNotShowed){ countersAnimation(); } } }

            $('.dropdown').click(function() {
                //console.log("Клик по dropdown");
                if( $(this).parent().children('ul').is(":visible") )
                {
                    //console.log("Сабменю видимо - закрываем сабменю");
                    $('.submenu').parent().children('ul').hide();
                    $(this).removeClass('active');
                }else{
                    //console.log("Сабменю скрыто - раскрываем сабменю");
                    $('.submenu').hide();
            ...
            $(document).mouseup( function(e){
                ...
                if ( !div.is(e.target) && !div2.is(e.target) && div.has(e.target).length === 0 ) {
                    //console.log("Клик НЕ по сабменю - закрываем сабменю");
                    $('.dropdown').removeClass('active');
                    div.hide();
                }
            });
            ...
            $(document).scroll(function ()
            {
                ...
                //console.log('значение select-1: ' + $('#_select_1').val());
                //console.log('значение select-2: ' + $('#_select_2').val());
                //console.log('значение select-2: ' + $('#_select_3').val());
